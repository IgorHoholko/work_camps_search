...
def parse_projects_urls(tree: etree._Element) -> List[str]:
    return tree.xpath("/html/body/div[1]/section/div[1]/div/div/div[2]/div[2]/article/div/div[2]/div/div/a/@href")


def parse_countries(tree: etree._Element) -> List[str]:
...
def parse_cities(tree: etree._Element) -> List[str]:
    cities = tree.xpath(
...
def parse_dates(tree: etree._Element) -> Tuple[List[Timestamp], List[Timestamp], List[Timedelta]]:
...
def parse_needed(tree: etree._Element) -> Tuple[List[int], List[int], List[int]]:
...
def parse_project_codes(tree: etree._Element) -> List[str]:
...
def parse_project_names(tree: etree._Element) -> List[str]:
...
def parse_project_descriptions(projects_urls: List[str], safe_mode: bool = True) -> Tuple[List[str], List[dict]]:
    raw_descs = []
...
def split_descriptions_dicts(descriptions_dicts: List[dict]) \
